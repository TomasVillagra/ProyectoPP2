...
import PedidoAgregarItemModal from "../../components/pedidos/PedidoAgregarItemModal";
import PedidoCancelarModal from "../../components/pedidos/PedidoCancelarModal";
...
async function validarStockItem({ id_plato, cantidad }) {
  ...
  if (plato) {
    ...
    if (stockPlato >= cantPedida) return null;
  }
  ...
  return {
    platoId,
    motivo: "Faltan insumos para elaborar el plato.",
    faltantes,
  };
}
...
export default function PedidosList() {
  ...
  const setMesaEstado = async (pedido, nombreEstadoMesa) => {
    try {
      ...
      if (!mesaId) return;
      const target = estadosMesa.find(
        (e) => clean(e.estms_nombre) === clean(nombreEstadoMesa)
      );
      if (!target) return;
      await api.patch(`/api/mesas/${mesaId}/`, {
        id_estado_mesa: Number(target.id_estado_mesa),
      });
    } catch (e) {
      console.warn(
  ...
  const marcarEntregado = async (p) => {
    try {
      ...
      const entregadoId = resolveEstadoId(
        estados,
        ["entregado"],
        FALLBACK_IDS.ENTREGADO
      );
  ...
  const marcarFinalizado = async (p) => {
    try {
      const id = p.id_pedido ?? p.id;
      const finalizadoId = resolveEstadoId(
        estados,
        ["finalizado"],
        FALLBACK_IDS.FINALIZADO
      );
      await api.patch(`/api/pedidos/${id}/`, {
        ped_fecha_hora_fin: nowForApiLocal(),
  ...
  const marcarCancelado = async (p, restarStock = false) => {
    try {
      const id = p.id_pedido ?? p.id;
      const canceladoId = resolveEstadoId(
        estados,
        ["cancelado", "anulado"],
        FALLBACK_IDS.CANCELADO
      );
      await api.patch(`/api/pedidos/${id}/`, {
        ped_fecha_hora_fin: nowForApiLocal(),
  ...
  const confirmarAgregarItem = async () => {
    ...
    if (err) {
      const plato = platos.find(
        (p) => getPlatoId(p) === Number(err.platoId)
      );
      const nombrePlato =
    ...
    try {
      ...
      if (descontarAhora) {
        try {
          await api.post(
            `/api/pedidos/${id_pedido}/descontar_insumos_detalle/`,
            {
              id_plato: Number(id_plato),
          ...
          if (status === 404) {
            ...
            alert(
              "No se pudo descontar stock ahora: " +
  ...
  const opcionesEmpleado = useMemo(() => {
    ...
    data.forEach((r) => {
      const emp =
        r.empleado_nombre ?? r.emp_nombre ?? r.id_empleado ?? "";
  ...
  const opcionesTipo = useMemo(() => {
    ...
    data.forEach((r) => {
      const tipo =
        r.tipo_nombre ?? r.tipped_nombre ?? r.id_tipo_pedido ?? "";
  ...
  const opcionesEstado = useMemo(() => {
    ...
    data.forEach((r) => {
      const est =
        r.estado_nombre ?? r.estped_nombre ?? r.id_estado_pedido ?? "";
  ...
  const filtrados = useMemo(() => {
    ...
    return data.filter((r) => {
      ...
      const inicio =
        r.ped_fecha_hora_ini ?? r.ped_fecha ?? r.fecha ?? null;
      const mesa = r.mesa_numero ?? r.ms_numero ?? r.id_mesa ?? null;
      const empleado =
        r.empleado_nombre ?? r.emp_nombre ?? r.id_empleado ?? "";
      const tipo =
  ...
  return (
    <DashboardLayout>
      ...
      <PedidosFilters
        ...
        opcionesMesa={opcionesMesa}
        opcionesEmpleado={opcionesEmpleado}
      ...
      {/* Modal Agregar Ítem (la lógica ya la tenías, solo envuelta en componente) */}
      <PedidoAgregarItemModal
        addOpen={addOpen}
        addTarget={addTarget}
        newItem={newItem}
        setNewItem={setNewItem}
        descontarAhora={descontarAhora}
        setDescontarAhora={setDescontarAhora}
        addMsg={addMsg}
        onConfirm={confirmarAgregarItem}
        onClose={() => setAddOpen(false)}
        platos={platos}
        platosConReceta={platosConReceta}
      ...
      <PedidoCancelarModal
        ...
        isEntregado={isEntregado}
        onCancelarSinStock={async () => {
          await marcarCancelado(cancelTarget, false);
          setCancelOpen(false);
        }}
        onCancelarConStock={async () => {
